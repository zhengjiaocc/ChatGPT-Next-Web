...
export async function POST(
  ...
  try {
    ...
    // replace mode: chunk 0 always carries the full first segment and sets the
    // baseline. append mode: subsequent chunks merge by id, with sort_ord offset
    // by the current array length so new messages land after existing ones.
    if (syncMode === "replace") {
      await sql`
            ...
            FROM (
              ...
              FROM (
                ...
                SELECT
                  ...
                  new_msg AS msg,
                  (jsonb_array_length(chat_sessions.messages) + new_ord)::bigint AS sort_ord,
                  1 AS is_new
                FROM jsonb_array_elements(EXCLUDED.messages) WITH ORDINALITY AS new_t(new_msg, new_ord)
              ) merged
              ORDER BY msg_id, is_new DESC
            ) dedup
          ),
          model      = EXCLUDED.model,
          mask       = EXCLUDED.mask,
