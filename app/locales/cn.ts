...
const cn = {
  ...
  Store: {
    ...
    Prompt: {
      ...
      Summarize:
        "请将对话压缩为可复用的长期上下文记忆。仅保留对后续有价值的信息：目标、关键约束、已确认决定、用户偏好、当前进展、未决问题与下一步。不要复述寒暄和无关细节，不要编造信息；不确定内容请标注“未确认”。不要包含任何敏感信息（如密钥、密码、令牌、Cookie、证件号、银行卡号），如出现请写为“[已省略敏感信息]”。请输出为结构化要点，长度尽量精炼但信息完整。",
    },
  },
  Copy: {
    Success: "已写入剪贴板",
