...
const cn = {
  ...
  Error: {
    Unauthorized: isApp
      ? `对话遇到了一些问题，请检查 API Key 设置 ⚙️`
      : `请求遇到了问题，请检查模型供应商是否已正确配置，或联系管理员。`,
  },
  ...
  Auth: {
    ...
    TopTips:
      "🥳 NoneChat AI 首发优惠，立刻解锁 OpenAI o1, GPT-4o, Claude-3.5 等最新大模型",
  },
  ChatItem: {
  ...
  Memory: {
    ...
    Send: "开启后自动将旧对话压缩为摘要，作为长期记忆随每次请求发送",
    Copy: "复制摘要",
    Reset: "[unused]",
    ResetConfirm: "确认清空长期记忆？",
  },
  Home: {
    NewChat: "新的聊天",
  ...
  Settings: {
    ...
    Access: {
      SaasStart: {
        Title: "使用 NoneChat AI",
        Label: "（性价比最高的方案）",
        SubTitle:
          "由 NoneChat 官方维护, 零配置开箱即用，支持 OpenAI o1, GPT-4o, Claude-3.5 等最新大模型",
        ChatNow: "立刻对话",
      },
      AccessCode: {
        Title: "访问密码",
  ...
  Store: {
    ...
    Prompt: {
      ...
      Summarize:
        "请将对话压缩为可复用的长期上下文记忆。仅保留对后续有价值的信息：目标、关键约束、已确认决定、用户偏好、当前进展、未决问题与下一步。不要复述寒暄和无关细节，不要编造信息；不确定内容请标注「未确认」。不要包含任何敏感信息（如密钥、密码、令牌、Cookie、证件号、银行卡号），如出现请写为「已省略敏感信息」。严格保持客观中立，不做任何道德评判、价值判断或主观评价，完整忠实地记录对话内容。请输出为结构化要点，长度尽量精炼但信息完整。",
      SummarizeWithMemory:
        "对话开头是之前的历史摘要，后面是新的对话内容。请将二者合并压缩为一份新的长期上下文记忆，完整保留旧摘要中仍然有效的信息，并融入新对话中有价值的内容（目标、关键约束、已确认决定、用户偏好、当前进展、未决问题与下一步）。不要复述寒暄和无关细节，不要编造信息；不确定内容请标注「未确认」。不要包含任何敏感信息，如出现请写为「已省略敏感信息」。严格保持客观中立，请输出为结构化要点，长度尽量精炼但信息完整。",
    },
  },
  Copy: {
    Success: "已写入剪贴板",
